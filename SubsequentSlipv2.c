...
/*
#ifdef GPU
} //#pragma acc data create(fx,fy,fz)
#endif
*/
	return;
}
...
#define		DELTA4(i,j,k,l) (((i==j) && (j==k) && (k==l))?1:0)
	...
	for (i=0; i<ND; i++) {
        ...
        #pragma acc kernels
        {
        #pragma acc loop gang worker(32) private(k1,k2,k3,m,n,u,v,i,j,k,l,fk,nfreq,fk4,G) collapse(3) /*tile(64,4,32) */ 
        #endif
	for(k1=0;k1<N1;k1++)
    {
		  ...
		  //#pragma acc loop private(k1,k2,k3,m,n,u,v,i,j,k,l,G,fk,nfreq,fk4,A)    
			...
			/*	#ifdef GPU
				...
				*/	
				...
				#ifdef GPU
				#pragma acc loop  vector private(k1,k2,k3,m,n,u,v,i,j,k,l,G,fk,nfreq,fk4) collapse(4)   
				#endif
					for (m=0; m<ND; m++) {
			        #ifdef GPUU
				...
				#pragma acc loop private(k1,k2,k3,m,n,u,v,i,j,k,l,G,fk,nfreq,fk4)       
												...
												for	(l=0; l<ND; l++) {
													...
													tmp+=- C[k][l][u][v]*C[i][j][m][n]*G[k][i]*fk[j]*fk[l] ;
												}
												A[k1][k2][k3][m][n][u][v] =tmp;
											}
										}
									}
