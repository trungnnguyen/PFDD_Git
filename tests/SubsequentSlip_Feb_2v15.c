...
printf("Number Of GPUs %d\n",num_devices);
  ...
  #ifdef GPU
  #pragma acc data  copy(data[0:2*(NSV)*(N1)*(N2)*(N3)],xi[0:2*(NSV)*(N1)*(N2)*(N3)-1],xi_bc[0:2*(NSV)*(N1)*(N2)*(N3)-1])  
  #pragma acc data   copyin(xi_o[0:(N1)*(N2)*(N3)])
  {
  #endif
...
void initial(float * data, double * xi, double * xi_bc, double setobs, int * xi_o,int border,int ppoint_x, int ppoint_y, int ppoint_z){
  ...
  #ifdef GPU
  #pragma acc data  present(data[0:2*(NSV)*(N1)*(N2)*(N3)],xi[0:2*(NSV)*(N1)*(N2)*(N3)-1],xi_bc[0:2*(NSV)*(N1)*(N2)*(N3)-1])  
  #pragma acc data   present(xi_o[0:(N1)*(N2)*(N3)])
  {
  #endif 
