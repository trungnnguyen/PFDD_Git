...
printf("Number Of GPUs %d\n",num_devices);
    ...
    //data =  (float*)malloc((2*(NSV)*(N1)*(N2)*(N3)+1)*sizeof(float));
    data =  new float[2*(NSV)*(N1)*(N2)*(N3)+1];
    //data2 = (float*) malloc((2*(NSV)*(N1)*(N2)*(N3)+1)*sizeof(float));
    data2=new float[2*(NSV)*(N1)*(N2)*(N3)+1];
    //datag=(float*)malloc((2*(NS*N1*N2*N3)+1)*sizeof(float));
    datag=new float[2*(NS*N1*N2*N3)+1];
    //databeta = (float*)malloc(2*((ND)*(ND)*(N1)*(N2)*(N3)+1)*sizeof(float));
    databeta=new float[2*((ND)*(ND)*(N1)*(N2)*(N3)+1)];
    //dataeps = (float*)malloc(2*((ND)*(ND)*(N1)*(N2)*(N3)+1)*sizeof(float));
    dataeps=new float[2*((ND)*(ND)*(N1)*(N2)*(N3)+1)];
    //dataepsd = (float*)malloc(2*((ND)*(ND)*(N1)*(N2)*(N3)+1)*sizeof(float));
    dataepsd=new float[2*((ND)*(ND)*(N1)*(N2)*(N3)+1)];
    //datasigma = (float*)malloc(2*((ND)*(ND)*(N1)*(N2)*(N3)+1)*sizeof(float));
    datasigma=new float[2*((ND)*(ND)*(N1)*(N2)*(N3)+1)];

    //sigmav =(float*) malloc(2*N1*N2*N3*NV*sizeof(float));
    sigmav=new float[2*N1*N2*N3*NV];
    //xi = (double*)malloc(2*(NSV)*(N1)*(N2)*(N3)*sizeof(double));
    xi=new double[2*(NSV)*(N1)*(N2)*(N3)];
    //xi_bc = (double*)malloc(2*NSV*N1*N2*N3*sizeof(double));
    xi_bc=new double[2*NSV*N1*N2*N3];
    //xi_sum = (double*)malloc(2*(N1)*(N2)*(N3)*sizeof(double));
    xi_sum=new double[2*(N1)*(N2)*(N3)];
    //fx = (double*)malloc((N1)*(N2)*(N3)*sizeof(double));
    fx=new double[(N1)*(N2)*(N3)];
    //fy = (double*)malloc((N1)*(N2)*(N3)*sizeof(double));
    fy=new double[(N1)*(N2)*(N3)];
    //fz = (double*)malloc((N1)*(N2)*(N3)*sizeof(double));
    fz=new double[(N1)*(N2)*(N3)];
    //f = (double*)malloc((NS)*(N1)*(N2)*(N3)*sizeof(double));
    f=new double[(NS)*(N1)*(N2)*(N3)];
    //r = (double*)malloc((ND)*sizeof(double));
    r=new double[(ND)];
    //BB =(double*) malloc((NS)*(NSV)*(N1)*(N2)*(N3)*sizeof(double));
    BB=new double[(NS)*(NSV)*(N1)*(N2)*(N3)];
    //FF = (double*)malloc((NS)*(N1)*(N2)*(N3)*(ND)*(ND)*sizeof(double));
    FF=new double[(NS)*(N1)*(N2)*(N3)*(ND)*(ND)];
    //DD = (double*)malloc((NSV)*(N1)*(N2)*(N3)*(ND)*(ND)*sizeof(double));
    DD=new double[(NSV)*(N1)*(N2)*(N3)*(ND)*(ND)];
    //FFv = (double*)malloc((NV)*(N1)*(N2)*(N3)*(ND)*(ND)*sizeof(double));
    FFv=new double[(NV)*(N1)*(N2)*(N3)*(ND)*(ND)];
    //sigmal= (double*)malloc((N1)*(N2)*(N3)*(ND)*(ND)*sizeof(double));
    sigmal=new double[(N1)*(N2)*(N3)*(ND)*(ND)];
    //GG = (double*)malloc((2*(NS)*(NS)*(N1)*(N2)*(N3))*sizeof(double));
    GG=new double[2*(NS)*(NS)*(N1)*(N2)*(N3)];
    //xi_o = (int*)malloc((N1)*(N2)*(N3)*sizeof(int));
    xi_o=new int[(N1)*(N2)*(N3)];
    //II = (double*)malloc((ND)*(ND)*(N1)*(N2)*(N3)*sizeof(double));
    II=new double[(ND)*(ND)*(N1)*(N2)*(N3)];
    //penetrationstress = (double*)malloc(N2*sizeof(double));// only valid for N2 = 2
    penetrationstress=new double[N2];
    //penetrationstress2 = (double*)malloc(N2*sizeof(double));// only valid for N2 = 2
    penetrationstress2=new double[N2];
  ...
  /* print files */
   ...
   // ofFinalXi = fopen("FinalXi.dat","rb");
    ...
    //testplastic =  (int*)malloc(plastic_max*sizeof(int));
    testplastic =  new int[plastic_max];
...
void dSmat(double Cm[NMAT][3], double Sm[NMAT][3], double AA[NMAT][ND][ND], double KK[NMAT][6][6], double dS[NMAT][6][6], int ZLdC[NMAT][6], int prop[NMAT][6],double theta1[NMAT][ND][ND]){
	...
	dC = (double**)malloc((6)*sizeof(double *));
	C0 = (double**)malloc((6)*sizeof(double *));
	for (i=0;i<6;i++){
		dC[i] = (double*)malloc((6)*sizeof(double));
		C0[i] = (double*)malloc((6)*sizeof(double));
	}
	...
	if(countr!=0){
	dCR = (double **)malloc((countr)*sizeof(double *));
	dSR = (double **)malloc((countr)*sizeof(double *));
	CR = (double **)malloc((countr)*sizeof(double *));
		K = (double **)malloc((countr)*sizeof(double *));
	for (i=0;i<countr;i++){
		dCR[i] = (double *)malloc((countr)*sizeof(double));
		dSR[i] = (double *)malloc((countr)*sizeof(double *));
		CR[i] = (double *)malloc((countr)*sizeof(double *));
		K[i] = (double *)malloc((countr)*sizeof(double *));
	}
		printf("dCR is \n");																								 
		i1=0;
		for(i=0;i<6;i++)
